...
function setup() {
  ...
  tower = new Tower(width/2 - 600 , height - 290, 170, 310);
...
function draw() {
  ...
  for (var i = 0; i < balls.length; i++) {
    ...
    for (var j = 0 ; j < boats.length; j++){
      if (balls[i] !== undefined && boats[j] !== undefined) { var collision = Matter.SAT.collides(balls[i].body, boats[j].body); if (collision.collided) { boats[j].remove(j); Matter.World.remove(world, balls[i].body); balls.splice(i, 1); i--; } }
    }
  }
  showBoats();
  cannon.display();
